...
const dbs = await client.query(
  q.Map(q.Paginate(q.Match(q.Index('one_vampire'), id)), (ref) => q.Get(ref))
);
...
await client.query(
  q.Create(q.Collection('vampires'), { data: { ...body, id } })
);
...
if (vampire.privateSheet) {
  ...
  if (
    !(await db.vampires.isEditorOrViewer(
      String(id),
      session.user.id,
